...
Description:
    ...
    - Logs timestamps (second precision) to output_log.txt.
    - Cuts must be at least 2 seconds apart to be recorded.
"""
...
MIN_SCENE_LENGTH_SEC = 2.0  # 컷 사이 최소 2초 이상
# ----------------------------


def timestamp_from_frame(frame_idx, fps, seconds_only=False):
        ...
        if is_cut_hist or is_cut_ssim:
            ...
            if current_time_sec - last_cut_time >= MIN_SCENE_LENGTH_SEC:
                scenes.append({
                    "type": "cut",
                    "frame": frame_idx,
                    "timestamp": timestamp_from_frame(frame_idx, fps, seconds_only=True)
                })
                ...
                if nonzero_count >= MIN_FADE_LENGTH_FRAMES:
                    ...
                    if len(scenes) == 0 or (scenes[-1]["type"] != "fade" or
                        ...
                        scenes.append({
                            ...
                            "start_time": timestamp_from_frame(int(fade_start_frame), fps, seconds_only=True),
                            "end_time": timestamp_from_frame(int(fade_end_frame), fps, seconds_only=True)
                        })
                        hist_diffs.clear()
                        ssim_vals.clear()
